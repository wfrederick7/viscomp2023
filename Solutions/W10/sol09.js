...
function rearrange(mesh) {
  ...
  for (let i = 0; i < mesh.indices.length; i += 3) {
      // Create a vec3 from the flat array
      var index = mesh.indices[i] * 3;
      var vertex = vec3.fromValues(mesh.vertices[index], mesh.vertices[index + 1], mesh.vertices[index + 2]);
  ...
  mesh.indices = newIndices_1.concat(newIndices_2, newIndices_3);
}
// == END TASK 1a == //
...
function getDecorativeBallCenters(mesh) {
  // Initialize sums and counts for each group
  var sumCoordsGroup1 = vec3.create(), countGroup1 = 0;
  var sumCoordsGroup2 = vec3.create(), countGroup2 = 0;
  var sumCoordsGroup3 = vec3.create(), countGroup3 = 0;

  for (let i = 0; i < mesh.vertices.length; i += 3) {
      // Create a vec3 from the flat array
      var vertex = vec3.fromValues(mesh.vertices[i], mesh.vertices[i + 1], mesh.vertices[i + 2]);

      // Categorize based on the Y-coordinate
      if (vertex[1] < -8) {
          vec3.add(sumCoordsGroup1, sumCoordsGroup1, vertex);
          countGroup1++;
      } else if (vertex[1] < 9) {
          vec3.add(sumCoordsGroup2, sumCoordsGroup2, vertex);
          countGroup2++;
      } else {
          vec3.add(sumCoordsGroup3, sumCoordsGroup3, vertex);
          countGroup3++;
      }
  }

  // Calculate the centers
  var centerGroup1 = countGroup1 > 0 ? vec3.scale(vec3.create(), sumCoordsGroup1, 1 / countGroup1) : vec3.create();
  var centerGroup2 = countGroup2 > 0 ? vec3.scale(vec3.create(), sumCoordsGroup2, 1 / countGroup2) : vec3.create();
  var centerGroup3 = countGroup3 > 0 ? vec3.scale(vec3.create(), sumCoordsGroup3, 1 / countGroup3) : vec3.create();
...
function sortDecorativeBalls(ballCoordinates, uModelViewMatrix) {
  ...
  var transformedBalls = ballCoordinates.map((coord, index) => {
      var transformedCoord = vec3.transformMat4(vec3.create(), coord, uModelViewMatrix);
      return { index: index, zValue: transformedCoord[2] };
  });

  // Sort the balls by their z values in ascending order (furthest first)
  transformedBalls.sort((a, b) => a.zValue - b.zValue);

  // Extract the sorted indices
  var sortedIndices = transformedBalls.map(ball => ball.index);
...
function main() {
  ...
  function render(now) {
    ...
    // Assuming each ball's indices are stored consecutively and occupy an equal portion of the array
    var indicesPerBall = mesh.indices.length / 3;
    orders.forEach(order => {
      // Calculate the starting point for the indices of each ball
      var offset = order * indicesPerBall;

      // Draw each ball
      // The offset in gl.drawElements is specified in bytes, so it needs to be multiplied by the size of the type
